...
class Npwizard {
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'verify_text'    => esc_html('verifying', 'antydankesalon2'),
    ...
    public function menu_page() {
        add_theme_page(esc_html__('Theme Wizard', 'antydankesalon2'), esc_html__('Theme Wizard', 'antydankesalon2'), 'manage_options', $this->page_slug, array($this, 'wizard_page'));
    }
    ...
    public function wizard_page() {
        ...
        <div class="wrap npwizard-wrap-perent">
            ...
            if (defined('APP_PLUGIN_VERSION') && isset($GLOBALS['npThemeVersion']) && (float)APP_PLUGIN_VERSION > (float)$GLOBALS['npThemeVersion']) {
                ...
                echo sprintf('<div class="npwizard-warning"><p>%s</p></div>', 'The active theme has a version lower than the plugin version. Please update the theme too.', 'antydankesalon2');
            }
            ?>
        </div><!-- .npwizard-wrap -->
    ...
    public function get_steps() {
        $steps = array(
            'done' => array(
                'id'			=> 'done',
                'title'			=> __('Your website is ready!', 'antydankesalon2'),
                'icon'			=> 'yes',
                'view'			=> 'get_step_done',
                'callback'		=> ''
            )
        ...
        $import_content_step = array(
            'content' => array(
                'id'			=> 'content',
                'title'			=> __('Import Content', 'antydankesalon2'),
                'icon'			=> 'welcome-content-menus',
                'view'			=> 'get_step_content',
                'callback'		=> 'import_content',
                'callback2'		=> 'replace_content',
                'button_text'	=> __('Import Content', 'antydankesalon2'),
                'button2_text'	=> __('Replace previously imported Content', 'antydankesalon2'),
                'can_skip'		=> true,
                'can_replace'	=> true
            ),
        );
        ...
        $import_products_step = array(
            'products' => array(
                'id' => 'products',
                'title' => __('Import Products', 'antydankesalon2'),
                'icon' => 'welcome-content-menus',
                'view' => 'getStepProducts',
                'callback_products_import' => 'import_products',
                'callback_products_replace' => 'replace_products',
                'button_products_import_text' => __('Import Products', 'antydankesalon2'),
                'button_products_replace_text' => __('Replace previously imported Products', 'antydankesalon2'),
                'can_skip' => true,
                'can_replace' => true
            ),
        );
        ...
        $install_plugin_step = array(
            'plugins' => array(
                'id'			=> 'plugins',
                'title'			=> __('Install Theme Plugins', 'antydankesalon2'),
                'icon'			=> 'admin-plugins',
                'view'			=> 'get_step_plugins',
                'callback'		=> 'install_plugins',
                'button_text'	=> __('Install Plugins', 'antydankesalon2'),
                'can_skip'		=> false
            ),
        );
    ...
    public function get_step_plugins() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Install plugins included with the Theme. <br>Click the "Install Plugins" button to start the installation.', 'antydankesalon2')
        );
    ...
    public function get_step_content() {
        ...
        if($hideImport) {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Content has already been imported. Please skip this step', 'antydankesalon2')
           );
        } else {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Theme has Pages, Images, Menu, Header, and Footer. </br></br>Do you want to import the Content?', 'antydankesalon2')
            );
        }
        $content['import_options'] = sprintf(
            ...
            __('Import Sidebars Content', 'antydankesalon2')
        );
        if (class_exists('Woocommerce')) {
            $content['import_options'] .= sprintf(
                ...
                __('Import Products Content to Woocommerce', 'antydankesalon2')
            );
        }
    ...
    public function getStepProducts() {
        ...
        if (file_exists($contentPath)) {
            ...
            if (!empty($data['Parameters']['productsJson'])) {
                ...
                if (!empty($productsData['products'])) {
                    $options .= '<option value="content-source" ' . selected(get_option('np_products_source'), 'content-source', false) . '>' . __('Content', 'antydankesalon2') . '</option>';
                }
            }
        }
        ...
        if (file_exists($themeProductsPath)) {
            ...
            if (!empty($data['products'])) {
                $options .= '<option value="theme-source" ' . selected(get_option('np_products_source'), 'theme-source', false) . '>' . __('Theme', 'antydankesalon2') . '</option>';
            }
        }

        $content['detail'] = '
    <div class="products-options">
        <label for="np_products_source">' . __('We have found Products that you can import to your site. </br></br>Do you want to continue with importing?', 'antydankesalon2') . '</label>
        </br></br>
    ...
    <p>';
        ...
        if (isset($old_products_data['products']) && count($old_products_data['products']) > 0) {
            $content['detail'] .= sprintf(
                '<p><b>%s</b></p>',
                __('Please note that some Products already exist in your database, and the import will overwrite the current ones.', 'antydankesalon2')
            );
        }
        return $content;
    }
    ...
    <div class="theme-appearance-options">
        <label for="np_theme_appearance">' . __('Choose the source for your site\'s Colors, Fonts, Header, and Footer, as well as for the Blog, E-Commerce, and other Templates.', 'antydankesalon2') . '</label>
        </br></br>
        <select name="np_theme_appearance" id="np_theme_appearance">
            <option value="theme-option" ' . selected(get_option('np_theme_appearance'), 'theme-option', false) . '>' . __('Theme', 'antydankesalon2') . '</option>
            <option value="plugin-option" ' . selected(get_option('np_theme_appearance'), 'plugin-option', false) . '>' . __('Plugin', 'antydankesalon2') . '</option>
        </select>
    </div>
    <p>' . sprintf(
                __('You can change it later in the <a href="%s">Nicepage Settings</a>', 'antydankesalon2'),
    ...
    public function get_step_done() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Congratulations! The theme has been activated and your website is ready.', 'antydankesalon2')
        );
        $content['summary'] .= sprintf( '<p>%s</p>', 'Create a new page with the Nicepage Editor.', 'antydankesalon2' );
	...
	public function setup_plugins() {
		if (! check_ajax_referer('npwizard_nonce', 'wpnonce') || empty($_POST['slug'])) {
			wp_send_json_error(array('error' => 1, 'message' => esc_html__('No Slug Found', 'antydankesalon2')));
		}
		$json = array();
		...
		foreach ($plugins['activate'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Activating Plugin', 'antydankesalon2'),
		...
		foreach ($plugins['update'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Updating Plugin', 'antydankesalon2'),
				);
				break;
			}
		}
		foreach ($plugins['install'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Installing Plugin', 'antydankesalon2'),
				);
				break;
			}
		}
		if ($json) {
			...
			wp_send_json(array('done' => 1, 'message' => esc_html__('Success', 'antydankesalon2')));
		}
		exit;
	}
    ...
    public function setup_content() {
        ...
        $json = array(
            ...
            'message'       => esc_html__('Import Content', 'antydankesalon2'),
       );
        if ($json) {
            $json['hash'] = md5(serialize($json)); // used for checking if duplicates happen, move to next plugin
            wp_send_json($json);
